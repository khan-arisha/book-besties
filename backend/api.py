...
class BestSellersAPI:
    ...
    def __init__(self):
        try:
            self.conn = psycopg2.connect(database = psqlConfig.database, user = psqlConfig.username, password = psqlConfig.password, host="localhost")
    ...
    def getBestsellersGivenYearRange(self, yearStart, yearEnd):
        ...
        curr.execute("SELECT DISTINCT title FROM weekRankings WHERE EXTRACT(year FROM week) BETWEEN {} AND {} ORDER BY title;".format(yearStart, yearEnd))
    ...
    def getNumberOneBestsellersGivenYearRange(self, yearStart, yearEnd):
        ...
        curr.execute("SELECT DISTINCT title FROM weekRankings WHERE EXTRACT(year FROM week) BETWEEN {} AND {} AND ranking = 1;".format(yearStart, yearEnd)) 
        result = curr.fetchall()
        curr.close()
    ...
    def getBooksByAuthor(self, author):
        ...
        curr.execute("SELECT title FROM bestsellersInfo WHERE author = '{}' ;".format(author))
        result = curr.fetchall()
        if not result:
            return "This author has no books on the list"
            
        curr.close()
        formattedBooksByAuthor = self.formatTuples(result)
        return formattedBooksByAuthor
           

        

    def getAuthorsWithMoreThanNumBooks(self, numBooks):
        ...
        if not self.numIsGreaterThanZero(numBooks):
            return "invalid value"
            
        curr = self.cursor_init()
        
        curr.execute("SELECT author FROM bestsellersperauthor WHERE numBestsellers >= {} ;".format(numBooks))
        result = curr.fetchall()
        curr.close()
        
        if not result:
            return "no authors have more than {} books on the bestseller list".format(numBooks)
            
        formattedAuthors = self.formatTuples(result)
    ...
    def getLongestRunningBooks(self, topNum):
        ...
        curr.execute("SELECT title, total_weeks FROM bestsellersInfo ORDER BY total_weeks DESC LIMIT {} ;".format(topNum))
        result = curr.fetchall()
        curr.close()
    ...
    def isBestseller(self, bookName):
        ...
        curr.execute("SELECT * FROM bestsellersInfo WHERE title = '{}';".format(bookName))
        result = curr.fetchall()
        curr.close()
        if not result:
            return False
    ...
    def getBookInfo(self, bookName):
        ...
        if not self.isBestseller(bookName):
            return "This book is not on the bestseller list"

        curr = self.cursor_init()
        curr.execute("SELECT author, bestRank, total_weeks, first_week FROM bestsellersInfo WHERE title = '{}';".format(bookName))
        result = curr.fetchall()
        curr.close()
        return result[0]
